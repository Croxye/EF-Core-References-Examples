...
var persons7 = await context.Persons.FromSql($"SELECT * FROM Persons").ToListAsync();
#endregion

#region Stored Procedure Execute
var personsP = await context.Persons.FromSql($"EXECUTE dbo.sp_GetAllPersons 4").ToListAsync();
#endregion

#region Parametreli Sorgu Oluşturma
#region Örnek 1
int personId = 3;
var persons1 = await context
    .Persons.FromSql($"SELECT * FROM Persons Where PersonId = {personId}")
    .ToListAsync();
...
var persons2 = await context
    .Persons.FromSql($"EXECUTE dbo.sp_GetAllPersons {personId2}")
    .ToListAsync();
#endregion
...
SqlParameter personId = new("PersonId", "3");
var persons = await context
    .Persons.FromSql($"EXECUTE dbo.sp_GetAllPersons {personId}")
    .ToListAsync();
#endregion
...
SqlParameter personId = new("PersonId", "3");
var persons = await context
    .Persons.FromSql($"EXECUTE dbo.sp_GetAllPersons @PersonId = {personId}")
    .ToListAsync();
#endregion
#endregion
#endregion
...
string columnName = "PersonId",
    value = "3";
var persons = await context
    .Persons.FromSql($"Select * From Persons Where {columnName} = {value}")
    .ToListAsync();
...
var persons = await context
    .Persons.FromSqlRaw($"Select * From Persons Where {columnName} = @PersonId", value)
    .ToListAsync();
...
var persons = await context
    .Persons.FromSql($"SELECT * FROM Persons")
    .Where(p => p.Name.Contains("a"))
    .ToListAsync();

var data = await context
    .Database.SqlQuery<int>($"SELECT PersonId Value FROM Persons")
    .Where(x => x > 5)
    .ToListAsync();
...
await context.Persons.FromSql($"SELECT Name, PersonId FROM Persons").ToListAsync();
...
await context.Persons.FromSql($"SELECT * FROM Persons").Include(p => p.Orders).ToListAsync();
#endregion
