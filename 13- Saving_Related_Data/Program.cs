...
protected override void OnModelCreating(ModelBuilder modelBuilder)
{
    // Configure the primary key for AuthorBook as a composite key.
    modelBuilder.Entity<AuthorBook>()
        .HasKey(ab => new { ab.BookId, ab.AuthorId });

    // Configure the one-to-many relationship between Author and AuthorBook.
    modelBuilder.Entity<AuthorBook>()
        .HasOne(ab => ab.Author) // Each AuthorBook has one Author.
        .WithMany(a => a.Books) // Each Author can have many AuthorBook entries.
        .HasForeignKey(ab => ab.AuthorId); // The foreign key in the AuthorBook entity pointing to the Author entity.

    // Configure the one-to-many relationship between Book and AuthorBook.
    modelBuilder.Entity<AuthorBook>()
        .HasOne(ab => ab.Book) // Each AuthorBook has one Book.
        .WithMany(b => b.Authors) // Each Book can have many AuthorBook entries.
        .HasForeignKey(ab => ab.BookId); // The foreign key in the AuthorBook entity pointing to the Book entity.
}
}
#endregion
