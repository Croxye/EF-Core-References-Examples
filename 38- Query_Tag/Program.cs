...
await context
    .Persons.TagWith("Tüm personeller çekilmişit.r")
    .Include(p => p.Orders)
    .TagWith("Personellerin yaptığı satışlar sorguya eklenmiştir.")
    .Where(p => p.Name.Contains("a"))
    .TagWith("Adında 'a' harfi olan personeller filtrelenmiştir.")
    .ToListAsync();
#endregion
...
await context
    .Persons.TagWithCallSite("Tüm personeller çekilmişit.r")
    .Include(p => p.Orders)
    .TagWithCallSite("Personellerin yaptığı satışlar sorguya eklenmiştir.")
    .Where(p => p.Name.Contains("a"))
    .TagWithCallSite("Adında 'a' harfi olan personeller filtrelenmiştir.")
    .ToListAsync();
#endregion
public class Person
{
...
class ApplicationDbContext : DbContext
{
    ...
    readonly ILoggerFactory loggerFactory = LoggerFactory.Create(
        builder =>
            builder
                .AddFilter(
                    (category, level) =>
